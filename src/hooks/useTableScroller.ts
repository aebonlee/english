...
interface UseTableScrollerOptions {
  scope?: string;
...
interface WrapperRecord {
  ...
  table: HTMLElement;
  parent: HTMLElement;
  shadowLeft: HTMLDivElement;
  shadowRight: HTMLDivElement;
  updateShadows: () => void;
}
...
export function useTableScroller({
...
}: UseTableScrollerOptions = {}): void {
  const wrappersRef = useRef<WrapperRecord[]>([]);
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      wrappersRef.current.forEach(({ wrapper: _wrapper, updateShadows }) => {
        updateShadows();
      });
    };
