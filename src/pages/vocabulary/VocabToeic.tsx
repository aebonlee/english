...
import { toeicWords } from '../../data/vocabulary';

const ITEMS_PER_PAGE = 20;
const TOTAL_LABEL = 800;
const ACCENT_COLOR = '#8E44AD';
const ACCENT_BG = '#f8f0fb';
const ACCENT_BORDER = '#d7b4e8';

export default function VocabToeic(): React.ReactElement {
  const { t } = useLanguage();
  useAOS();
  ...
  const toggleLearned = (e: React.MouseEvent, id: number) => {
    e.stopPropagation();
  ...
  return (
    <>
      <SEOHead
        title={t('TOEIC 필수 단어 800 - English Pro', 'TOEIC Essential 800 - English Pro')}
        description={t(
          'TOEIC 시험에 자주 출제되는 핵심 어휘 800개를 학습하세요.',
          'Study 800 essential TOEIC vocabulary words for high scores.'
        )}
      />
      ...
      <section className="page-header" data-aos="fade-up">
        <div className="container">
          <div className="page-header__breadcrumb">
            ...
            <i className="fas fa-chevron-right"></i>
            <span>{t('TOEIC 필수 단어', 'TOEIC Essential')}</span>
          </div>
          <h1 className="page-header__title">
            {t('TOEIC 필수 단어 800', 'TOEIC Essential 800')}
          </h1>
          ...
          <div className="vocab-progress">
            <div className="vocab-progress-bar">
              <div
                ...
                style={{ width: `${(learnedCount / TOTAL_LABEL) * 100}%`, background: `linear-gradient(90deg, ${ACCENT_COLOR}, #6C3483)` }}
              />
            </div>
      ...
      {hasMore && (
        <div className="vocab-load-more">
          <button onClick={handleShowMore} style={{ borderColor: ACCENT_COLOR, color: ACCENT_COLOR }} onMouseEnter={(e) => { (e.target as HTMLElement).style.background = ACCENT_COLOR; (e.target as HTMLElement).style.color = '#fff'; }} onMouseLeave={(e) => { (e.target as HTMLElement).style.background = '#fff'; (e.target as HTMLElement).style.color = ACCENT_COLOR; }}>
      ...
      <style>{`
        ...
        @media (max-width: 600px) { .vocab-grid { grid-template-columns: repeat(2, 1fr); gap: 8px; } }
        .vocab-card { perspective: 800px; height: 140px; cursor: pointer; }
        ...
        .vocab-card-front, .vocab-card-back { position: absolute; inset: 0; backface-visibility: hidden; -webkit-backface-visibility: hidden; border-radius: 10px; padding: 12px; display: flex; flex-direction: column; border: 1px solid #e8e8e8; background: #fff; overflow: hidden; align-items: center; justify-content: center; text-align: center; }
        .vocab-card-back { transform: rotateY(180deg); }
        .card-top-row { position: absolute; top: 8px; left: 10px; right: 10px; display: flex; justify-content: space-between; align-items: center; }
        .category-badge { font-size: 0.6rem; font-weight: 600; text-transform: uppercase; background: #eef2f7; color: #6b7c93; padding: 2px 7px; border-radius: 8px; letter-spacing: 0.5px; }
        .learn-check { background: none; border: 2px solid #ccc; border-radius: 50%; width: 22px; height: 22px; font-size: 11px; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #ccc; transition: all 0.2s; padding: 0; line-height: 1; }
