...
import { proseWorks, levelLabels } from '../../data/literatureData';
import type { LiteratureWork } from '../../data/literatureData';
...
interface TTSReturn {
  ...
  speak: (paragraphs: string[], workId: string) => void;
  pause: () => void;
  resume: () => void;
  stop: () => void;
}

function useTTS(): TTSReturn {
  ...
  const speak = useCallback((paragraphs: string[], workId: string) => {
    stop();
    setPlaying(workId);
...
function LiteratureCard({ work, tts, t, language }: LiteratureCardProps): React.ReactElement {
...
export default function ShortStories(): React.ReactElement {
  ...
  return (
    <>
      <SEOHead
        ...
        description={t(
          '이솝 우화, O. 헨리, 링컨·처칠 명연설을 원문·번역·TTS로 학습하세요.',
          'Study classic fables, short stories, and famous speeches with text, translation, and TTS.'
        )}
      />
