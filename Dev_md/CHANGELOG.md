...
- 10개 항목 점수화 평가 (종합 89.4/100, B+)
- `Dev_md/SITE_EVALUATION.md` 신규 생성
- 사이트 규모 현황, 항목별 강점/개선점, 우선 개선 로드맵 포함
...
**통합 트리 네비게이션:**
- 데스크톱: 2차 메뉴(SubNav) + 3차 메뉴(섹션 앵커)를 사이드바 통합 트리로 구성
- 카테고리 헤더 토글(접기/펼치기) → 2차 메뉴 항목 나열
- 현재 페이지 아래에 3차 메뉴(섹션 앵커) 드롭다운 자동 표시
- 비활성 항목 클릭 시 페이지 이동, 활성 섹션 자동 추적
- 모바일: 기존 SubNav 가로 바 + SectionNav 칩 유지

**SubNav.jsx:**
- `navData`, `categoryTitles` export 추가

**PageLayout.jsx:**
- `category` prop 추가, 통합 트리 구조 (2차+3차 메뉴 일체형)
- `category` 없는 페이지는 기존 ToC 목차 fallback

**CSS (`site.css`):**
- `.sidebar-nav` 카테고리 네비게이션 스타일
- `.sidebar-nav__sections` / `.sidebar-nav__section-link` 3차 메뉴 스타일
- `.toc-list` gap `4px`, `.toc-link` padding `10px 14px`, `line-height: 1.5`
- `.sub-nav-mobile` 데스크톱 숨김 / 모바일 표시
- `.page-header__en` 영문 0.7em 축소 스타일

**다크모드:**
- `.sidebar-nav`, `.sidebar-nav__sections`, `.sidebar-nav__section-link` 다크 오버라이드

**16개 페이지 JSX:**
- SubNav import 제거, `category` prop을 PageLayout에 전달

**10개 타이틀:**
- 영문 괄호 부분 `<span className="page-header__en">` 래핑 (0.7em)
